...
public protocol AsyncReader<ReadElement, ReadFailure>: ~Copyable, ~Escapable {
    ...
    associatedtype ReadIterator: BorrowMutableIteratorProtocol<ReadElement> & ~Copyable & ~Escapable
...
extension AsyncReader where Self: ~Copyable, Self: ~Escapable {
    ...
    public mutating func read<Return, Failure: Error>(
        ...
        do {
            return try await self.read { (iterator) throws(Failure) -> Return in
                return try await body(&iterator)
            }
        } catch {
            switch error {
            case .first:
